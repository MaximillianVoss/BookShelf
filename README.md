...
В ветке `codex/api-sources-no-key` Google Books отключен. Все внешние источники работают без API-ключа, OAuth, токенов и антибот-проверок. В режиме `Все` приложение запрашивает каждый внешний API отдельно, удаляет дубликаты и показывает общий список. Если внешние источники недоступны или вернули пустой результат, используется локальный набор книг.
