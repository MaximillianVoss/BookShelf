...
- Выбор источника поиска: все источники, Open Library, Gutendex, Internet Archive, Library of Congress или локальный каталог.
...
В ветке `api-sources-no-key` Google Books отключен. Все внешние источники работают без API-ключа, OAuth, токенов и антибот-проверок. В режиме `Все` приложение запрашивает каждый внешний API отдельно, удаляет дубликаты по названию и первому автору, затем показывает общий список. Если внешние источники недоступны или вернули пустой результат, используется локальный набор книг.
