...
- Выбор источника поиска: все источники, Open Library, Open Library HTML, Яндекс Книги или локальный каталог.
...
- Яндекс Книги HTML: `https://books.yandex.ru/`
- локальный fallback-каталог внутри приложения

В ветке `codex/html-parser-source` Google Books API отключен. Для режима `Все` приложение сначала использует Open Library API, затем добирает результат HTML-парсерами Open Library и Яндекс Книг. Если внешние источники недоступны или вернули пустой результат, используется локальный набор книг.

HTML-парсинг зависит от верстки сайта и может временно возвращать пустой список, если сайт изменил разметку или ограничил обычные HTTP-запросы.
...
| Интеграции | Open Library API, Open Library HTML, Яндекс Книги HTML, fallback-каталог |
